...
def create_task():
    ...
    tasks.append(task)
    # Clear cache for GET /tasks since task list has changed
    cache.delete('tasks')
    return jsonify({"message": "Task created", "task": task}), 201

# GET /tasks - Fetch all tasks with caching
...
@cache.cached(timeout=300, key_prefix=lambda: hashlib.md5(request.get_data()).hexdigest())
def query_tasks():
    data = request.get_json()
